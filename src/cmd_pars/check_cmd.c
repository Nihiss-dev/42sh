...
** Last update Wed Jun 11 09:10:03 2014 NicolasOVEJERO
*/
...
{
  if (nb_car_spe(tab_sd[re], PIPE) == 0 && i == 1)
    {
      if (nb_car_spe(tab_sd[re], RED_G) == 0)
...
int	other(char **tab_sd, t_gene *utils, int i, int re)
{
  ...
  if  ((ret = top(tab_sd, utils, i, re)) == 10)
    {
      if (nb_car_spe(tab_sd[re], PIPE) != 0)
